class CachingUtils {
  ...
  static stringifyQueryOptions(queryOpt) {
    ...
    Object.entries(queryOpt).forEach(([key, value]) => {
      ...
      if (key === "where" || key === "include") {
        queryOptString +=
          ":" + key + JSON.stringify(CachingUtils.desymbolize(value));
      } else {
    ...
    Object.keys(data).forEach((key) => {
      ...
      if (Array.isArray(value)) {
        try {
          ...
          value.forEach((nestedValue, i) =>
            CachingUtils.restoreTimestamps(nestedValue, nestedInstances[i])
          );
        } catch (error) {
      ...
      if (typeof value === "object") {
        try {
          ...
          Object.values(value).forEach((nestedValue) =>
            CachingUtils.restoreTimestamps(nestedValue, nestedInstance)
          );
        } catch (error) {
          // TODO: Fix issue with JSON and BLOB columns
        }
  ...
  static generateIncludeRecurse(model, depth = 1) {
    ...
    return Object.entries(model.associations || [])
      .filter(([as, association]) => {
        const hasOptions = Object.prototype.hasOwnProperty.call(
          association,
          "options"
        );
        return hasOptions;
      ...
      .map(([as, association]) => {
        ...
        return {
          ...
          include: CachingUtils.generateIncludeRecurse(
            associatedModel,
            depth + 1
          ),
          as,
  ...
  static getInstanceCacheKey(instance) {
    return CachingUtils.getInstanceModel(instance).primaryKeyAttributes.map(
      (pk) => instance[pk]
    );
  }
  ...
  static async save(cacheClient, instance, customKey) {
    ...
    const key = [CachingUtils.getInstanceModel(instance).name];
    ...
    return await cacheClient
      .set(key, CachingUtils.instanceToData(instance))
      .then(() => instance);
  }
  ...
  static async saveHash(cacheClient, instance, compositeKey) {
    ...
    //   .then(() => instance);
    return await cacheClient
      .hset(hashKey, key, CachingUtils.instanceToData(instance))
      .then(() => instance);
  }
  ...
  static async saveAll(cacheClient, model, instances, customKey) {
    ...
    //   .then(() => instances)
    return await cacheClient
      .set(key, instances.map(CachingUtils.instanceToData))
      .then(() => instances);
  }
  ...
  static async saveHashMultiKeys(cacheClient, model, instances, compositeKey) {
    ...
    //   .then(() => instance);
    return await cacheClient
      .hsetMultiKeys(hashKey, instancesObject)
      .then(() => instances);
  }
  ...
  static async saveHashAndCountAll(
    cacheClient,
    model,
    countAndInstances,
    compositeKey
    ...
    //   .then(() => instances)
    return await cacheClient
      .hset(hashKey, compositeKey.key, JSON.stringify(countAndInstances))
