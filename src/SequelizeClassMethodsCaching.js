...
class SequelizeClassMethodsCaching {
  ...
  static buildClassMethodsForCaching(
    cacheClient,
    sequelizeModel,
    loggedInUserId,
    cid
    ...
    return {
      ...
      async create() {
        ...
        compositeKey["hashKey"] = loggedInUserId
          ? loggedInUserId + ":" + sequelizeModel.name + ":findByPk"
          : sequelizeModel.name + ":findByPk";
        compositeKey["key"] = cid ? cid : "";
        let instance = await sequelizeModel.create.apply(
          sequelizeModel,
          arguments
        );
      ...
      async bulkCreate() {
        ...
        compositeKey["hashKey"] = loggedInUserId
          ? loggedInUserId + ":" + sequelizeModel.name + ":findByPk"
          : sequelizeModel.name + ":findByPk";
        let instances = await sequelizeModel.bulkCreate.apply(
          sequelizeModel,
          arguments
        );
        if (instances) {
          await CachingUtils.saveHashMultiKeys(
            cacheClient,
            sequelizeModel,
            instances,
            compositeKey
      ...
      async findByPk(id) {
        ...
        return await CachingUtils.getHash(
          cacheClient,
          sequelizeModel,
          compositeKey
        ).then((instance) => {
          ...
          return (sequelizeModel.findByPk || sequelizeModel.findById)
            .apply(sequelizeModel, arguments)
            .then((instance) => {
      ...
      async update(data) {
        ...
        let instance = await sequelizeModel.update.apply(
          sequelizeModel,
          arguments
        );
        // if update successfully on database
        if (instance[0] === 1) {
          await CachingUtils.clearHashKey(
            cacheClient,
            sequelizeModel,
          ...
          await CachingUtils.clearKey(cacheClient, sequelizeModel, deleteKey);
          if (instance[1])
            return await CachingUtils.saveHash(
              cacheClient,
              sequelizeModel.build(instance[1][0].dataValues),
              compositeKey
            );
      ...
      async findAll(queryOptions) {
        let queryOptiontString = queryOptions
          ? CachingUtils.stringifyQueryOptions(queryOptions)
          : "";
        let compositeKey = {
          hashKey: "",
          key: "",
        ...
        if (instances) {
          ...
          let instances = await sequelizeModel.findAll.apply(
            sequelizeModel,
            arguments
          );
          return await CachingUtils.saveHashAll(
            cacheClient,
            sequelizeModel,
            instances,
            compositeKey
      ...
      async findAndCountAll(queryOptions) {
        ...
        if (countAndInstances) {
          // any array - cache hit
          // console.log(instances);
          return countAndInstances;
        } else {
          return await sequelizeModel.findAndCountAll
            .apply(sequelizeModel, arguments)
            .then((countAndInstances) => {
              return CachingUtils.saveHashAndCountAll(
                cacheClient,
                sequelizeModel,
                countAndInstances,
                compositeKey
      ...
      async findOne(queryOptions) {
        ...
        if (instance) {
          ...
          let instance = await sequelizeModel.findOne.apply(
            sequelizeModel,
            arguments
          );
          return await CachingUtils.saveHash(
      ...
      async destroy() {
        ...
        let deleteKey = loggedInUserId
          ...
          : sequelizeModel.name + ":find";
        if (compositeKey["key"] === "") {
          return await CachingUtils.clearKey(
            cacheClient,
            sequelizeModel,
            compositeKey.hashKey
          ).then(() => {
            return CachingUtils.clearKey(
              cacheClient,
              sequelizeModel,
              deleteKey
            );
          ...
          return await CachingUtils.clearHashKey(
            cacheClient,
            sequelizeModel,
            compositeKey.hashKey,
            compositeKey.key
...
module.exports = SequelizeClassMethodsCaching;
