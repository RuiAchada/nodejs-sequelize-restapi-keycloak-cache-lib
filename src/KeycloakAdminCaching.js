...
class KeycloakAdminCaching {
  ...
  static async keycloakAdminCache(
    method,
    keycloakAdminClient,
    moduleName,
    keycloakAdminClientFuncName,
    userIdLoggedIn,
    queryParams,
    bodyParams
  ) {
    ...
    let hashKey = [userLoggedIn, module];
    let kcAdminClient = keycloakAdminClient;

    let func_string =
      KeycloakAdminCaching.variableToString({ kcAdminClient }) +
      "." +
      keycloakAdminClientFuncName;
    ...
    switch (method) {
      case "GET":
        ...
        let key =
          keycloakAdminClientFuncName + ":" + JSON.stringify(queryParams);
        let cacheData =
          await KeycloakAdminCaching.redisKeycloakAdminClient.hget(
            hashKey,
            key
          );
        ...
        const result = await eval(
          func_string + "(" + JSON.stringify(queryParams) + ")"
        );

        // set cache for this get function
        await KeycloakAdminCaching.redisKeycloakAdminClient.hset(
          hashKey,
          key,
          result
        );
...
module.exports = KeycloakAdminCaching;
